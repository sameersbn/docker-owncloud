...
```bash
docker pull sameersbn/owncloud:9.0.3-1
```
...
docker run --name owncloud -itd --restart=always \
  ...
  sameersbn/owncloud:9.0.3-1 app:owncloud
```
...
docker run --name owncloud-nginx -itd --restart=always \
  ...
  --publish 10080:80 \
  sameersbn/owncloud:9.0.3-1 app:nginx
```
...
docker run --name owncloud -it --rm [OPTIONS] \
  sameersbn/owncloud:9.0.3-1 app:backup:create
```
...
```bash
docker run --name owncloud -it --rm [OPTIONS] \
  sameersbn/owncloud:9.0.3-1 app:backup:restore
```
...
```bash
docker run --name owncloud -it --rm [OPTIONS] \
  sameersbn/owncloud:9.0.3-1 app:backup:restore BACKUP=1417624827_owncloud_backup.tar
```

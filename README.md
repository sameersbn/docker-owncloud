...
```bash
docker pull sameersbn/owncloud:latest
```
...
docker run --name owncloud -it --rm [OPTIONS] \
  sameersbn/owncloud:latest app:backup:create
```
...
```bash
docker run --name owncloud -it --rm [OPTIONS] \
  sameersbn/owncloud:latest app:backup:restore
```
...
```bash
docker run --name owncloud -it --rm [OPTIONS] \
  sameersbn/owncloud:latest app:backup:restore BACKUP=1417624827_owncloud_backup.tar
```
